...
export class Client extends BaseClient<true> {
    ...
    embeds = new Collection<
        string,
        { embed: EmbedBuilder; interactionId: string }
    >()
    reconect = true
    _wsw = false
    ...
    constructor(options: ClientOptions) {
        ...
        this.oldCommands = new OldCommandManager(
            this,
            options.routes.oldCommands
        )
        ...
        this.once('ready', () =>
            this._onReady({
                eventsPath:
                    options.routes?.events ?? join(__dirname, '../events')
            })
        )
    ...
    private async _onReady(options: { eventsPath: string }) {
        ...
        console.log(
            '\x1b[31m%s\x1b[0m',
            `${this.user?.username} ${this.version} Lista y Atenta!!!`
        )
    }
    ...
    private _onWebSocketMessage(message: string): void {
        try {
            ...
            if ((error as string).startsWith('SyntaxError'))
        ...
        return Promise.all(
            readdirSync(path)
                ...
                .map(async file => {
                    ...
                    this.on(eventName as string, (...args) =>
                        event.default(...args)
                    )
                })
        )

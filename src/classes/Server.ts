...
export class Server {
    ...
    async syncDB(dataPriority?: boolean): Promise<any> {
        if (
            this.guild.client.user?.id === '897298074915975269' &&
    ...
    toDBObject(toPublic?: boolean): GuildDataBaseModel {
        ...
        if (
            JSON.stringify(this.getPrefixes(true)) !==
            JSON.stringify(['?', '>'])
        )
        ...
        if (this.#prefixes.includes(prefix)) {
            ...
            if (this.#prefixes.length < 1) {
                ...
                this.db
                    ...
                    .catch(() =>
                        this.db.set({ prefixies: FieldValue.delete() })
                    )
            } else
                this.db
    ...
    addSuggestChannel(suggestChannelObject: SuggestChannelObject) {
        if (suggestChannelObject.default)
            this.suggestChannels = this.suggestChannels.map(i =>
                i.default ? suggestChannelObject : i
            )
        ...
        this.db
            ...
            .catch(() =>
                this.db.set({ suggest_channels: this.suggestChannels })
            )
        ;(this.guild.client as Client).websocket?.send(
            JSON.stringify({
    ...
    setMessageUpdateLog(channel: string) {
        ...
        this.db
            ...
            .catch(() =>
                this.db.set({ ['logs_channels.message_update']: channel })
            )
        ;(this.guild.client as Client).websocket?.send(
            JSON.stringify({
                event: 'set_log',
    ...
    setMessageDeleteLog(channel: string) {
        ...
        this.db
            ...
            .catch(() =>
                this.db.set({ ['logs_channels.message_delete']: channel })
            )
        ;(this.guild.client as Client).websocket?.send(
            JSON.stringify({
                event: 'set_log',
    ...
    setAttachmentLog(channel: string) {
        ...
        this.db
            ...
            .catch(() =>
                this.db.set({ ['logs_channels.message_attachment']: channel })
            )
        ;(this.guild.client as Client).websocket?.send(
            JSON.stringify({
                event: 'set_log',
    ...
    startEmojiAnalisis() {
        ...
        this.emojiTimeout = setInterval(() => {
            this.db
                ...
                .catch(() =>
                    this.db.set({ emoji_statistics: this.emojiStatistics })
                )
        }, 600_000)
        ...
        this.guild.client.on('messageCreate', msg => {
            ...
            const ids = emojis?.map(e =>
                e.replace(/<a?:[a-z_]+:(?<id>\d{18})>/i, '$<id>')
            )
    ...
    addAutorol(name: string, id: string) {
        ...
        return this.db
            .update({
                ['autoroles.' + name]: Array.from(
                    this.autoroles.get(name)!.values()
                )
            })
            .catch(() =>
                this.db.set({
    ...
    removeAutorol(name: string) {
        ...
        return this.db
            ...
            .catch(() =>
                this.db.set({ ['autoroles.' + name]: FieldValue.delete() })
            )
    }
    ...
    setInviteChannel(inviteChannel: string) {
        ...
        this.db
            ...
            .catch(() =>
                this.db.set({ ['logs_channels.invite']: inviteChannel })
            )
        ;(this.guild.client as Client).websocket?.send(
            JSON.stringify({
                event: 'set_log',
    ...
    addBlacklistedWord(word: string) {
        ...
        this.db
            ...
            .catch(() =>
                this.db.set({ ['blacklisted_words']: this.blacklistedWords })
            )
        ;(this.guild.client as Client).websocket?.send(
            JSON.stringify({
    ...
    removeBlacklistedWord(word: string) {
        this.blacklistedWords = this.blacklistedWords.filter(
            dbWord => dbWord !== word
        )
        this.db
    ...
    addDisabledChannel(channelID: string) {
        ...
        this.db
            ...
            .catch(() =>
                this.db.set({ ['disabled_channels']: this.disabledChannels })
            )
        ;(this.guild.client as Client).websocket?.send(
            JSON.stringify({
    ...
    sendSuggestion(
        ...
        if (interaction instanceof ChatInputCommandInteraction) {
            channel = (interaction.options.getChannel('channel') ??
                client.channels.cache.get(
                    this.suggestChannels[0]?.channel as string
                )) as TextChannel
        ...
        const embed = new EmbedBuilder()
            ...
            .setTitle(
                this.translate('suggest_cmd.title', { id: this?.lastSuggestId })
            )
            .setColor(16313844)
    ...
    async createCommands(commands: ApiCommand[]) {
        for (const command of commands) {
            fetch(
                `https://discord.com/api/v10/guilds/${this.guild.id}/commands`,
                {
                    method: 'POST',
                    headers: {

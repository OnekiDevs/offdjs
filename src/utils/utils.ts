...
export function permissionsError(
    ...
    interaction.reply({
        content: `No tienes los permissions suficientes, necesitas \`${new PermissionsBitField(
            permissions
        )
            .toArray()
            .join(', ')}\``,
...
export async function sendError(error: Error, file: string) {
    ...
    const channel = await client.channels.fetch(
        client.constants.errorChannel as string
    )
    if (channel)
...
export const Translator = function (
    interaction: BaseInteraction | Message<true>
) {
    let lang: string =

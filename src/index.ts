import { ApplicationCommandDataResolvable, IntentsBitField, ClientOptions, Client, Events } from 'discord.js'
    ...
    override async login(token?: string, verbose = false): Promise<string> {
        ...
        try {
            ...
            for (const file of readdirSync(join(process.cwd(), this.options.root, 'commands'))) {
                const cmd = (await import(join(process.cwd(), this.options.root, 'commands', file))) as {
            ...
            for (const guild of this.guilds.cache.values()) {
                ...
                for (const [guilds, command] of guildCommandsData)
                    if (!guilds.length || guilds.includes(guild.id)) toSend.push(command)
        ...
        this.on(Events.InteractionCreate, interaction => {
            ...
            if (interaction.isButton())
                return buttonsCache
                    ...
                    .forEach(h => h(interaction, ...interaction.customId.split(':')))

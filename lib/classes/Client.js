...
import { CommandManager, ServerManager, ComponentManager, OldCommandManager, Server } from '../utils/classes.js';
    ...
    async _checkBirthdays() {
        ...
        usersSnap.forEach(async (user) => {
            ...
            this.servers.map(async (server) => {
                ...
                if (!member)
                    member = await server.guild.members.fetch(user.id);
                if (!member)
                    return;
                let channel = server.guild.channels.cache.get(birthdayChannel);
                if (!channel)
                    channel = (await server.guild.channels.fetch(birthdayChannel));
                if (!channel)
                    return server.removeBirthdayChannel();
                channel.send(server.birthday.message?.replaceAll('{username}', `<@${user.id}>`) ??
